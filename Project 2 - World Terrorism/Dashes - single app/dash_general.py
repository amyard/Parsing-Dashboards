...
def map():
    ...
    for i in df['inj-group'].unique():
        map=df[(df['inj-group']==i)&(df['Year']==2014)]
        trace = dict(
                   type = 'scattergeo',
                   locationmode = 'world',
                   showlegend = True,
                   ...
                   lon = map['longitude'],
                   lat = map['latitude'],
                   text = '{}/{}/{}<br>{}<br>{}<br>{}'.format(map['Day'],map['Month'],map['Year'],
                                                      map['city'],map['Killed'],map['Wounded']),
                   mode = 'markers',
                   hoverinfo = 'text',
                   marker = dict(opacity = 0.8, size=2)
                   )
...
app.layout = html.Div(style={'width':'1800', 'height':'1000', 'background':'black'}, children = [
    # div for title
    html.H1(children = 'SOME GENERAL INFORMATION', style={'textAlign':'center', 'color':'white', 'padding-top':'50'}),

    # Slider for year
    # html.Div(style = {'width':'49%', 'border':'1px solid green'}, children =
    #     dcc.Slider(id = 'year--slider', min = df['Year'].min()-1, max = df['Year'].max(), value = df['Year'].max(),
